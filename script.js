...
// ========== ОТКРЫТИЕ ВЫПАДАЮЩЕГО МЕНЮ ПО КЛИКУ НА МОБИЛЬНЫХ ==========
...
document.addEventListener('click', function(e) {
    if (!e.target.closest('.nav-item.dropdown') && !e.target.closest('.mobile-toggle')) {
        document.querySelectorAll('.nav-item.dropdown').forEach(item => item.classList.remove('active'));
        if (menu && window.innerWidth <= 768) menu.classList.remove('active');
    }
});
...
function populateServiceSelects(category = 'all') {
    ...
    const selects = [
        ...
        document.getElementById('service-select-useful')
    ];
    selects.forEach(select => {
        ...
        if (select.id === 'service-select-form') {
            ...
            emptyOption.value = ''; emptyOption.textContent = '-- Выберите услугу --';
            emptyOption.disabled = true; emptyOption.selected = true;
...
function renderPortfolio(category = 'all') {
    ...
    portfolioGrid.innerHTML = filtered.map(item => `
        <div class="portfolio-card">
            <div class="portfolio-card__image"><img src="${item.img}" alt="${item.title}" loading="lazy"><span class="portfolio-card__category">${getCategoryName(item.category)}</span></div>
            <div class="portfolio-card__content"><h3 class="portfolio-card__title">${item.title}</h3><p class="portfolio-card__desc">${item.desc}</p><div class="portfolio-card__meta"><span><i class="fas fa-map-marker-alt"></i> Кострома</span><span><i class="fas fa-check-circle"></i> Выполнено</span></div></div>
        </div>
    `).join('');
}
function getCategoryName(cat) { const names = { facade: 'Фасад', roof: 'Кровля', mount: 'Монтаж', other: 'Прочее' }; return names[cat] || cat; }
document.querySelectorAll('.filter-btn').forEach(btn => btn.addEventListener('click', () => { document.querySelectorAll('.filter-btn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); renderPortfolio(btn.dataset.filter); }));
...
if (reviewsContainer) {
    ...
    function renderReviews() {
        reviewsContainer.innerHTML = reviews.map((r, i) => `<div class="review-card" style="display: ${i === currentReview ? 'block' : 'none'};"><div class="review-card__avatar">${r.name.charAt(0)}</div><div class="review-card__name">${r.name}</div><div class="review-card__stars">${'★'.repeat(r.stars)}${'☆'.repeat(5 - r.stars)}</div><div class="review-card__text">«${r.text}»</div></div>`).join('');
        if (sliderDots) sliderDots.innerHTML = reviews.map((_, i) => `<button class="slider-dot ${i === currentReview ? 'active' : ''}" data-index="${i}"></button>`).join('');
    }
    function showReview(index) {
        currentReview = (index + reviews.length) % reviews.length;
        reviewsContainer.querySelectorAll('.review-card').forEach((card, i) => card.style.display = i === currentReview ? 'block' : 'none');
        if (sliderDots) sliderDots.querySelectorAll('.slider-dot').forEach((dot, i) => dot.classList.toggle('active', i === currentReview));
    }
    ...
    sliderDots?.addEventListener('click', (e) => { if (e.target.classList.contains('slider-dot')) showReview(parseInt(e.target.dataset.index)); });
    renderReviews();
...
// ========== ОТКРЫТИЕ ВКЛАДКИ ПО URL ==========
function openTabFromUrl() { const params = new URLSearchParams(window.location.search); const tab = params.get('tab'); if (tab) { const tabBtn = document.querySelector(`.service-tab-btn[data-tab="${tab}"]`); if (tabBtn) setTimeout(() => tabBtn.click(), 100); } }
